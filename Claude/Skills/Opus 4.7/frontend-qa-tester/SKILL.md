...
description: 'Use when planning, creating, running, or repairing frontend QA coverage for browser UI: end-to-end flows, accessibility checks, visual regression, responsive viewport checks, keyboard/interaction tests, cross-browser checks, smoke tests, or mock-vs-implementation comparison. Triggers on requests such as "QA this UI", "test this flow", "write Playwright tests", "run an accessibility audit", "check this against the mock", "add visual regression", "test responsive breakpoints", or "debug this frontend test failure". Do not use for production implementation, static mock creation, backend/API contract work, non-browser unit tests, performance benchmarking alone, CI pipeline architecture alone, or legal accessibility certification.'
---
...
1. **Plan the QA work.** Use `TodoWrite` for multi-flow, high-risk, cross-browser, visual-regression, accessibility, or failing-test investigations. Track scope, project inspection, dependency readiness, test changes, validation, cleanup, and handoff.
