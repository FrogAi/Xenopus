...
description: Audits project dependencies for vulnerabilities, license risk, version drift, transitive bloat, and supply-chain risk across npm/yarn/pnpm, pip/uv/poetry, Go modules, Cargo, Maven/Gradle, RubyGems, Composer, NuGet, and system package manifests. Use for "audit dependencies," "check vulnerabilities," "CVE scan," "license audit," "GPL/AGPL/SSPL check," "lockfile drift," "outdated dependencies," "supply-chain audit," "typosquat," "dependency confusion," or "reconcile scanner output." Avoid applying upgrades, editing lockfiles, choosing new dependencies, CI scan integration, IaC dependency review, and legal conclusions.
when_to_use: Use when the user wants dependency inventory, advisory reconciliation, license classification, outdated/EOL package review, package provenance review, or scanner-output reconciliation. Do not use for making dependency upgrades, refactoring code for a vulnerable API, designing CI gates, auditing container images only, or giving binding legal advice.
argument-hint: "[repo/path, ecosystem, mode: full/cve/license/drift/supply-chain/reconcile, policy]"
---
...
- Reconcile every material dependency finding against at least one current primary advisory, registry, package metadata, scanner output, or lockfile source.
- Verify scanner outputs are read-only, deduplicated by stable advisory/package/version identity, and not blindly trusted over source authorities.
- Check that all credential-like package-manager config values are redacted and reported only by path and secret class.
- State unscanned ecosystems, missing lockfiles, unauthenticated private registries, stale scanner databases, and external data-egress decisions.
- Confirm no manifest, lockfile, package cache, build artifact, or dependency pin was mutated.
...
Use the listed authorities as starting points. During real dependency work, refresh the source that controls the ecosystem or claim being used.
