...
description: Use when measuring performance of code paths, functions, scripts, CLI commands, local or authorized HTTP endpoints, services, or SQL queries. Triggers on benchmark, measure performance, compare speed, p95 or p99 latency, throughput, before/after timing, benchmark suite, or CI performance regression. Do not use for code optimization itself, profiler-style hotspot attribution, unauthorized production load testing, security testing, or server tuning; route those to the relevant specialist skill.
---
