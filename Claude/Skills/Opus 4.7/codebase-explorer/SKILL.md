...
description: Maps, traces, and explains a repository from local evidence so Claude can answer architecture, entry-point, data-flow, onboarding, and change-location questions without guessing.
when_to_use: Use when the user asks to explore a repo, map a codebase, explain architecture, find entry points, trace a feature or request flow, identify data flow, create an onboarding tour, locate where to add a feature, find complexity or churn hotspots, draw a Mermaid or C4-style diagram, or answer structural questions about repository organization. Do not use for implementation planning, symbol-level downstream impact, performance benchmarking, security testing, AGENTS.md authoring, skill authoring, or applying code changes.
argument-hint: "[repo path, question, feature flow, role, or output location]"
---
...
- **Hotspot:** Code that appears risky because of local evidence such as churn, complexity, size, centrality, flaky tests, ownership ambiguity, or repeated task-marker comments.
...
8. **Assess hotspots when relevant.** Combine git churn, file size, complexity indicators, test gaps, task-marker density, centrality, dependency fan-in/fan-out, and recent failure evidence. Label approximations.
...
User asks where the repo is risky. Combine churn, file size, central imports, task-marker comments, test coverage clues, and recent failures if available. Rank with confidence and route fixes to the right skill.
