...
description: Use when debugging, validating, replaying, tuning, or investigating openpilot/FrogPilot routes, drive logs, rlog/qlog files, cereal events, modelV2, controlsState, carState, livePose, liveCalibration, planning, steering, longitudinal control, perception, calibration, route regressions, telemetry, or real-data validation for openpilot/FrogPilot code changes. Triggers on "debug my route", "analyze this drive", "replay this route", "validate against logs", "check this on a drive", "openpilot logs", "FrogPilot route", "comma route", "model is doing X", "wrong sign", "tune from data", or "before deploying verify against logs". Do not use for generic Linux debugging, openpilot onboarding, casual architecture questions without route/code evidence, or code review that has no route/log validation surface.
---
...
1. **Plan the work.** Use `TodoWrite` for any route analysis, control change, tuning, telemetry, or deploy-prep task.
...
- Claude Code skills documentation, `https://code.claude.com/docs/en/skills`. Used for this skill's Claude routing and frontmatter behavior.
