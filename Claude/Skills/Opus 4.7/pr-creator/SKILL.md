---
name: pr-creator
description: 'Use when drafting, creating, updating, or preparing a pull request or merge request: title, body, summary, test plan, risk, rollback, screenshots, linked issues, reviewers, labels, or branch/base readiness. Triggers on requests such as "draft a PR", "create a pull request", "open an MR", "write the PR description", "update the PR body", "summarize this branch for review", or "prepare this branch for GitHub/GitLab review". Do not use for code review findings, release notes, commit-message rewriting alone, frontend QA itself, implementation work, incident response, or ticket-management writes.'
---
...
1. **Plan the PR work.** Use `TodoWrite` for non-trivial branches, high-risk changes, external PR/MR creation, multi-repo work, or ambiguous scope. Track evidence collection, draft, validation, platform action, and cleanup.
...
11. **Handle rollback concretely.** State the real rollback path: revert commit/merge, feature flag, config rollback, database forward recovery, deploy rollback, migration reversal, or "no simple rollback" with mitigation. Do not use a vague fallback; name the exact rollback action that evidence supports.
