...
description: "Use for server/runtime config: nginx, Caddy, Apache, HAProxy, Envoy, TLS, caching, systemd, Docker, Kubernetes, and OS limits."
---
...
Use the exact platform's current official or primary docs during the task. Start with the source map entry matching the server, proxy, runtime, orchestrator, or hosting platform in scope:
...
- **Validation:** commands run, results, skipped checks with reason, dry-run or syntax status, and post-change read-back for every live or local config mutation that can be safely inspected.
