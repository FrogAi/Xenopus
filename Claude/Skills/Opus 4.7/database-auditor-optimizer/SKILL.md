...
description: 'Audits and optimizes relational databases: PostgreSQL, MySQL, MariaDB, SQLite, Supabase Postgres, Aurora PostgreSQL, and Aurora MySQL. Use for schema review, query-plan analysis, index recommendations, slow-query triage, ORM N+1 detection, migration safety, RLS policy review, backup/restore posture, table bloat, vacuum/analyze, connection pooling, locks, replication lag, and safe database-change planning. Avoid NoSQL/document/graph/time-series databases, database vendor selection, pure application code optimization, server/OS tuning, and live database mutations unless explicitly requested and gated.'
when_to_use: Use when the user wants relational database schema, SQL, migrations, query performance, indexes, RLS, backups, locks, pooling, or live database safety reviewed or improved. Do not use for non-relational databases, ORM choice debates, app-layer fixes without DB evidence, cloud infrastructure changes outside the database, or generic data analysis.
argument-hint: "[engine/version, schema/query/migration/path, environment, mode: audit/optimize/migration/rls/backup/pool]"
---
...
11. **Validate.** Re-run the strongest check matching the changed surface: plan analysis for query shape, catalog checks for schema state, migration dry runs for DDL/data changes, tests for application behavior, and safe post-change reads for applied live work. For live mutations, verify the database state after execution.
...
Use the listed authorities as starting points. During real database work, refresh the source that controls the target engine, provider, or feature.
