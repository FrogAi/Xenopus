...
description: "Use for AGENTS.md, CLAUDE.md, and editor rules: loading, precedence, conflicts, and authoring. Not for skills or READMEs."
---
...
1. **Plan the work.** Maintain an explicit plan/checklist for multi-file, migration, reconciliation, diagnosis, or write tasks. Track scope, runtime-source checks, repo evidence, draft/fix, validation, and cleanup.
...
11. **Handle Claude and Codex differences.** Use current docs rather than memory. As a current baseline to verify at invocation:
    - Codex uses `AGENTS.md` and supported configured instruction-discovery behavior. Do not assume Codex reads `CLAUDE.md` unless current docs or local config prove an import, fallback, or supported loading path.
    - Claude Code uses `CLAUDE.md` and its own supported memory/import/discovery behavior. Do not assume Claude Code reads `AGENTS.md` unless current docs or local config prove an import or supported loading path.
