...
description: Investigates a specific bug from symptom to verified root cause, then routes or prepares the smallest correct fix path without masking the issue.
when_to_use: Use when the user asks why a test fails, why an endpoint returns an error, why behavior changed, why a regression happened, why a flaky test fails, why a crash occurs, why a wrong value is returned, or to find the root cause of a named bug, stack trace, log, Sentry/Dynatrace event, CI failure, or repro case. Do not use for broad codebase exploration, general code review, feature work, pure performance optimization without a concrete bug, or writing tests after the root cause is already known.
argument-hint: "[symptom, failing command, stack trace, log, event ID, expected vs actual, or regression range]"
---
...
Stay in the investigation layer while this skill is active. Do not edit files solely because this skill was invoked. When the same user request asks to fix the bug, complete the root-cause investigation first, then route execution to `surgical-change-finder`, `code-optimizer`, `migration-writer`, `test-creator`, or direct tools based on the confirmed owner layer and selected fix path.
