...
description: Performs source-grounded research, claim validation, comparison, lineage tracing, and synthesis for topics outside the local codebase. Use for "research X," "deep research," "fact-check this claim," "compare sources," "what is the current state of X," "trace the history of X," "what does the literature say," or "synthesize these findings." Avoid internal company docs, local codebase exploration, legal document review, code security testing, incident forensics, prompt/skill authoring, and pure opinion writing.
when_to_use: Use when the user needs evidence-backed research from web, primary sources, papers, standards, official docs, public records, or user-provided documents. Do not use for codebase-internal research, connector-only company knowledge search, implementation planning without external research, or domains with a more specific active skill.
argument-hint: "[topic, claim, comparison set, time window, source standard, or downstream decision]"
---
...
- **Mutable claim:** A claim likely to change over time, including current versions, prices, laws, APIs, practice guidance, product features, company facts, model capabilities, benchmarks, schedules, and security guidance.
...
During use, the relevant topic sources become the evidence set. Refresh official or primary sources in the same invocation when claims are mutable, high stakes, contested, or likely to affect a decision.
