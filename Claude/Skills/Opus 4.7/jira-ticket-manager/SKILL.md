...
description: Requires a configured Atlassian/Jira MCP or connector; if missing or unauthenticated, report the blocker and required user action instead of attempting Jira work. Searches, reads, audits, drafts, and safely mutates Jira issues when that connector is available. Use for "search Jira," "read ABC-123," "JQL query," "create a Jira ticket," "update this ticket," "comment on ABC-123," "transition to In Progress," "assign this ticket," "link blockers," "log work," "find my open tickets," or "audit sprint/backlog health." Avoid Confluence content, non-Jira trackers, PR drafting, incident coordination, and unsupported attachment or destructive operations.
when_to_use: Use when Jira is the issue source of truth or target mutation surface and a Jira connector is configured, or when the correct response is to report missing Jira connector/auth setup. Do not use for Confluence pages, Slack threads, GitHub/Linear/Asana/ServiceNow issues, PR descriptions, or project planning that only consumes Jira context.
argument-hint: "[issue key/project/JQL/query/create/update/comment/transition/link/worklog/audit scope]"
---
