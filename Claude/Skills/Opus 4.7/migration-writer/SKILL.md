...
description: Writes safe local migration artifacts for database schema/data changes, dependency upgrades, framework migrations, API contract migrations, cross-language ports, and ETL/data-shape moves. Use for "write a migration," "add/drop/rename this column," "write a backfill," "create rollback migration," "migrate API v1 to v2," "upgrade this framework," "migrate from X to Y," "write expand-contract plan," or "make this migration production-safe." Avoid live production apply, generic schema audits without writing, query optimization, incident response, and package audits without applying upgrades.
when_to_use: Use when the user wants migration files, rollback files, backfill scripts, upgrade steps, compatibility shims, deprecation plans, or migration verification commands written or reviewed. Do not use for merely auditing schema/query performance, applying migrations to production, choosing a new architecture, or coordinating an incident caused by a migration.
argument-hint: "[migration goal, system/framework/engine, target files, environment, constraints]"
---
...
6. **Choose migration strategy.** Match the strategy to the safety class: additive/expand-contract for compatibility, online DDL for lock-sensitive schema changes, chunked backfills for large data movement, dual-read/dual-write for transition windows, feature flags for rollout control, compatibility shims for mixed-version clients, and phased deprecation for removals.
...
Use the listed authorities as starting points. During real migration work, refresh the source that controls the target engine/framework/tool.
