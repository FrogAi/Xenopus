...
description: Traces symbol-level downstream impact for a proposed code, API, schema, config, or contract change without applying the change.
when_to_use: Use when the user asks what depends on a symbol, who calls a function, where a class or interface is implemented, what breaks if a function/type/endpoint/schema/config key is renamed, changed, or deleted, which consumers use an API contract, or what the blast radius of a specific change is. Do not use for broad repo tours, implementation planning, root-cause debugging, performance work, security testing, migration writing, or applying the change.
argument-hint: "[symbol, endpoint, schema field, config key, intended change, and repo scope]"
---
