---
name: plan-creator
description: Builds, refines, decomposes, sequences, and audits execution plans for Claude Code, Codex, subagents, or another agent runtime to execute with minimal hidden judgment.
when_to_use: Use when the user asks for a plan, implementation plan, migration plan, refactor plan, bug-fix plan, investigation plan, execution checklist, plan audit, plan refinement, plan decomposition, step sequencing, or a plan that a weaker model or fresh session must execute reliably. Do not use for human-facing project summaries, one-off answers, ordinary task execution, Claude Code skill authoring, Codex skill authoring, AGENTS.md authoring, prompt building, or subagent spawn orchestration.
argument-hint: "[goal, target runtime, scope, paths, constraints, and output location if writing]"
---
...
- **Target runtime:** The executor that will follow the plan, such as Claude Code, Codex, an Octopus agent, a subagent, a smaller model, a low-effort model, a future fresh session, or a human operator.
...
- Put research findings before the step that needs them. Do not use a vague research prerequisite without naming the exact source and fact to resolve.
- Give every branch an observable condition. Name the signal that determines whether the branch executes.
...
- There are no vague placeholders for validation, branching, edge coverage, or research scope.
...
- Static source list with verified-on dates for mutable facts.
...
- Claude Code skills documentation, `https://code.claude.com/docs/en/skills`. Used for skill frontmatter, lifecycle, allowed tools, sidecars, model/effort inheritance, and invocation behavior.
- Claude Code subagents documentation, `https://code.claude.com/docs/en/sub-agents`. Used for delegation boundaries, skill preloading implications, fork behavior, and subagent limitations.
- Anthropic Claude prompting guidance, `https://platform.claude.com/docs/en/build-with-claude/prompt-engineering/claude-prompting-best-practices`. Used for clear instructions, sequential steps, explicit output constraints, examples, and context discipline.
- OpenAI reasoning guidance, `https://developers.openai.com/api/docs/guides/reasoning-best-practices`. Used for planning-vs-execution separation, accuracy/reliability tradeoffs, clear constraints, and multistep planning guidance.
