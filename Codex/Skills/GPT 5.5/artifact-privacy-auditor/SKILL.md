...
description: "Use to audit local artifacts, logs, exports, screenshots, and caches for privacy, secrets, retention, and sharing risk."
---
...
4. **Inspect content minimally.** Read only enough content to classify risk. Use parsers or targeted scans when practical. Avoid opening credential stores, browser databases, chat histories, or binary blobs unless the task specifically requires and the privacy risk is justified. Credential-store exclusions must explicitly include `auth.json`, `credentials.json`, `.credentials.json`, token caches, provider auth files, session databases, and OS keychain material.
...
- A file appears to be a credential store, including `auth.json`, `credentials.json`, `.credentials.json`, token caches, browser storage databases, OS keychain material, session databases, or provider auth files.
