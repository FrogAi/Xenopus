...
description: Use when creating or revising browser-reviewable frontend mocks, wireframes, static prototypes, screen concepts, UI flow previews, or design handoff artifacts from a brief, screenshot, existing product surface, or project context before production implementation. Produces static HTML/CSS by default, or a framework-styled mock only when the user asks and the project already uses that stack. Triggers on requests such as "mock up this UI", "wireframe this page", "draft a static prototype", "design this screen", "make a browser preview", or "turn this idea into a UI mock". Do not use for production implementation, Figma source-of-truth design work, QA-only testing, visual regression suites, API/backend work, copy-only writing, or final brand identity.
---
...
1. **Plan the work.** Use `TodoWrite` for multi-screen, high-risk, ambiguous, or production-facing mocks. Keep the plan tied to observable deliverables: artifact inventory, source inspection, draft, validation, cleanup, and handoff.
