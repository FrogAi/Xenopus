...
description: Audits, hardens, creates, modifies, optimizes, or migrates CI/CD pipeline configuration for GitHub Actions, GitLab CI, CircleCI, Jenkins, Azure Pipelines, Bitbucket Pipelines, Buildkite, Drone, AWS CodeBuild, and AWS CodePipeline. Use for "audit my CI pipeline," "review this GitHub Actions workflow," "harden CI/CD," "fix pipeline YAML," "create a deploy workflow," "migrate CircleCI to GitHub Actions," "audit workflow secrets," "set up OIDC," "pin actions/images," "least-privilege runner permissions," "pipeline provenance," "SLSA," or "OWASP CI/CD review." Avoid build-tool tuning, test-framework design, app debugging, production incident triage, deploy-target infrastructure configuration, and live pipeline operations unless explicitly requested.
when_to_use: Use when the user wants CI/CD pipeline files or pipeline architecture reviewed, created, modified, optimized, hardened, or migrated. Do not use for a single failed job log without pipeline-configuration work, dependency vulnerability review outside CI/CD, cloud IAM implementation outside pipeline auth, release notes, server tuning, or generic security testing.
argument-hint: "[repo/path, CI platform, pipeline file, mode: audit/create/modify/harden/optimize/migrate, constraints]"
---
...
- **Source freshness:** Current official docs, platform schemas, validator behavior, security guidance, advisory feeds, and supply-chain standards confirmed during the task.
...
Use the listed authorities as starting points. During real CI/CD work, refresh the source that controls the platform or claim being used.
