...
description: Use when drafting, writing, updating, or publishing release notes, changelog entries, version summaries, GitHub/GitLab release bodies, customer announcements, internal release recaps, or breaking-change migration notes. Triggers on requests such as "draft release notes", "write the changelog entry", "prepare v1.2.3", "publish a GitHub release", "summarize this release", "create migration notes", or "turn merged PRs into release notes". Do not use for individual PR descriptions, code review, implementation work, commit-message rewriting alone, marketing campaigns beyond a release announcement draft, ticket-management writes, or incident communications.
---
...
1. **Plan the release work.** Use `TodoWrite` for non-trivial releases, multi-artifact releases, breaking changes, public announcements, release publication, or ambiguous source ranges. Track source range, convention detection, evidence, drafting, validation, side effects, and cleanup.
